...
# Switch a repo's origin to the new URL and reset to the default branch.
switch_remote() {
    repo_dir="$1"
    new_url="$2"
    branch="$3"
    ...
    # Reset tracking branch. Discard local changes (these are managed repos, not user code).
    if git -C "$repo_dir" rev-parse --verify "origin/$branch" >/dev/null 2>&1; then
        ...
        info "  Reset to origin/$branch"
    else
        warn "  Branch '$branch' not found on new remote. Manual intervention needed."
    fi
}
...
if is_fork_remote "$KLIPPER_DIR" "$FORK_KLIPPER_PATTERNS"; then
    switch_remote "$KLIPPER_DIR" "$MAINLINE_KLIPPER" "master"
else
...
if is_fork_remote "$MOONRAKER_DIR" "$FORK_MOONRAKER_PATTERNS"; then
    switch_remote "$MOONRAKER_DIR" "$MAINLINE_MOONRAKER" "master"
else
...
if [ ! -f "$MOONRAKER_CONF" ]; then
    ...
    if grep -q '\[update_manager kaos\]' "$MOONRAKER_CONF" 2>/dev/null; then
        ...
        info "  Adding [update_manager kaos] to moonraker.conf"
...
KAOS_EOF
        if grep -q '\[update_manager kaos\]' "$MOONRAKER_CONF" 2>/dev/null; then
            ...
            warn "  Failed to add update_manager section"
        fi
    fi
fi
echo ""
